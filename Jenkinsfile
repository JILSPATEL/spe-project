            ...
            steps {
                withCredentials([file(credentialsId: 'env-file', variable: 'ENV_FILE')]) {
                    script {
                        ...
                        if (!env.IMAGE_BACKEND?.contains('/')) {
                            error "IMAGE_BACKEND must include a Docker Hub namespace, e.g. speecommproject/backend"
                        }
                        ...
                        echo "Backend image prefix: ${env.IMAGE_BACKEND}"
            ...
            steps {
                withCredentials([usernamePassword(
                    ...
                    script {
                        ...
                        echo "Backend image namespace: $BACKEND_NAMESPACE"
                        echo "Frontend image namespace: $FRONTEND_NAMESPACE"
                        if [ "$BACKEND_NAMESPACE" != "$USER" ] || [ "$FRONTEND_NAMESPACE" != "$USER" ]; then
                            echo "WARNING: Docker login user and image namespace differ. The credential must have write access to these namespaces."
                        fi
                        '''
                        def servicesList = env.SERVICES.split(' ')
                        for (int i = 0; i < servicesList.size(); i++) {
